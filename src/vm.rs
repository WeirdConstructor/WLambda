...
macro_rules! out_reg {
    ($env: ident, $ret: ident, $retv: ident, $prog: ident, $respos_var: ident, $val: expr) => {
        match $respos_var {
...
macro_rules! op_r {
    ($env: ident, $ret: ident, $retv: ident, $prog: ident, $r: ident, $block: block) => {
        {
            let res = $block;
...
macro_rules! op_a {
    ($env: ident, $ret: ident, $retv: ident, $prog: ident, $a: ident, $block: block) => {
        {
...
macro_rules! op_a_r {
    ($env: ident, $ret: ident, $retv: ident, $prog: ident, $a: ident, $r: ident, $block: block) => {
        {
            in_reg!($env, $ret, $prog, $a);
            let res = $block;
            out_reg!($env, $ret, $retv, $prog, $r, res);
...
macro_rules! op_a_b_r {
    ($env: ident, $ret: ident, $retv: ident, $prog: ident, $a: ident, $b: ident, $r: ident, $block: block) => {
        {
            in_reg!($env, $ret, $prog, $a);
...
macro_rules! op_a_b_c_r {
    ($env: ident, $ret: ident, $retv: ident, $prog: ident, $a: ident, $b: ident, $c: ident, $r: ident, $block: block) => {
        {
            in_reg!($env, $ret, $prog, $a);
            in_reg!($env, $ret, $prog, $b);
    ...
    loop {
        ...
        match op {
            Op::S(op) => {
                match op {
                    OpS::ClearLocals(f, t) => {
                        env.push_clear_locals(*f as usize, *t as usize);
                    },
                    OpS::Accumulator(typ) => {
                        let v =
                            match typ {
            ...
            Op::A(op, a) => op_a!(env, ret, retv, prog, a, {
                match op {
                    OpA::Destr(info) => {
                        info.destructure(env, a);
                    },
                    OpA::JmpIfN(offs) => {
                        if !a.b() { pc = (pc as i32 + *offs) as usize; }
                    },
                    OpA::Break => {
                        let a = Box::new(a);
            ...
            Op::AR(op, a, r) => op_a_r!(env, ret, retv, prog, a, r, {
                match op {
                    OpAR::Mov    => a,
                    OpAR::NewOpt => VVal::Opt(Some(Rc::new(a))),
                    OpAR::NewErr =>
                        VVal::err(a, prog.debug[pc].clone().unwrap()),
                    OpAR::NewClos => {
                        let fun = a.clone_and_rebind_upvalues(|upvs, upvalues| {
                    ...
                    OpAR::GetSym2(sym_bx) => {
                        handle_err!(a, "map/list", retv)
                        .get_key(&sym_bx.0).unwrap_or_else(|| VVal::None)
                        .get_key(&sym_bx.1).unwrap_or_else(|| VVal::None)
                    },
                    OpAR::GetSym3(sym_bx) => {
                        handle_err!(a, "map/list", retv)
                        .get_key(&sym_bx.0).unwrap_or_else(|| VVal::None)
            ...
            match syn {
                ...
                Syntax::Opt => {
                    if let Some(v) = ast.at(1) {
                        ...
                        pw_store_if_needed!(prog, store, {
                            prog.op_new_none_opt(&spos, store);
                        })
                    }
                },
                Syntax::Map => {
