...
macro_rules! in_reg {
    ($env: ident, $ret: ident, $data: ident, $respos_var: ident) => {
        let $respos_var = match $respos_var {
...
macro_rules! out_reg {
    ($env: ident, $ret: ident, $retv: ident, $data: ident, $respos_var: ident, $val: expr) => {
        match $respos_var {
            ...
            ResPos::Global(i) => {
                $data[*i as usize].set_ref($val);
            }
            ResPos::GlobalRef(i) => {
                $data[*i as usize].deref().set_ref($val);
            }
            ResPos::Data(i) => {
                $data[*i as usize].set_ref($val);
            }
            ...
            ResPos::Value(ResValue::None) => {
                if let VVal::Err(ev) = $val {
                    $retv = Err(StackAction::panic_str(
...
macro_rules! call_ud_method {
    ($ud: ident, $key: ident, $argc: ident, $env: ident, $retv: ident, $uw_depth: ident, $prog: ident, $pc: ident, $call_ret: ident, $cont: block) => {{
        let call_ret = $env.with_local_call_info($argc, |env| $ud.call_method($key, env));
        match call_ret {
            Ok($call_ret) => {
                $env.popn($argc);
                $cont
...
pub fn vm(prog: &Prog, env: &mut Env) -> Result<VVal, StackAction> {
    ...
    let old_loop_info = std::mem::replace(&mut env.loop_info, LoopInfo::new());
    ...
    loop {
        let op = &prog.ops[pc];
        if DEBUG_VM {
            let syn = if let Some(sp) = &prog.debug[pc] { sp.s_short() } else { "".to_string() };
            println!(
                "OP[{:<2} {:>3}]: {:<40}      | sp: {:>3}, bp: {:>3}, uws: {:>3} | {}",
                env.vm_nest,
                pc,
                format!("{:?}", op),
                env.sp,
        ...
        match op {
            ...
            Op::NewNVec(vp, r) => match vp.as_ref() {
                NVecPos::IVec2(a, b) => {
                    in_reg!(env, ret, data, b);
                    in_reg!(env, ret, data, a);
            ...
            Op::ToRef(a, r, trtype) => match trtype {
                ToRefType::CaptureRef => match a {
                ...
                ToRefType::ToRef => op_a_r!(env, ret, retv, data, a, r, { a.to_ref() }),
                ToRefType::Hidden => {
                    op_a_r!(env, ret, retv, data, a, r, { a.to_hidden_boxed_ref() })
                }
                ToRefType::Weak => op_a_r!(env, ret, retv, data, a, r, { a.downgrade() }),
                ToRefType::Deref => op_a_r!(env, ret, retv, data, a, r, { a.deref() }),
            },
            ...
            Op::IterNext(ivar) => {
                ...
                let value = if let Some((v, k)) = i.borrow_mut().next() {
                    if let Some(k) = k {
                        VVal::pair(v, k)
                    } else {
                        v
            ...
            Op::GetIdx(o, idx, r) => op_a_r!(env, ret, retv, data, o, r, {
                handle_err!(o, "map/list", retv).at(*idx as usize).unwrap_or(VVal::None)
            }),
            ...
            Op::GetSym(o, sym, r) => op_a_r!(env, ret, retv, data, o, r, {
                handle_err!(o, "map/list", retv).get_key_sym(sym).unwrap_or(VVal::None)
            }),
            ...
            Op::CallDirect(fun) => {
                let a = &fun.arg;
                let r = &fun.res;
                op_a_r!(env, ret, retv, data, a, r, { (fun.fun)(a, env) })
            }
            ...
            Op::JmpTbl(a, tbl) => {
                ...
                let i = a.i();
                let idx: usize =
                    if i >= tbl.len() as i64 || i < 0 { tbl.len() - 1 } else { i as usize };
                pc = (pc as i32 + tbl[idx]) as usize;
            }
            Op::CtrlFlow(flw) => match flw {
                CtrlFlow::Next => {
                    handle_next!(env, pc, uw_depth, retv);
                }
                CtrlFlow::Break(a) => {
            ...
            Op::Builtin(b) => match b {
                ...
                Builtin::DumpVM(spos) => {
                    ...
                    for (i, op) in prog.ops.iter().enumerate() {
                        let syn = if let Some(sp) = &prog.debug[i] {
                            sp.s_short()
                        } else {
                            "".to_string()
                        };
        ...
        if DEBUG_VM {
            ...
            println!(
                "  loopinfo: uw={},sp={},pc={},bpc={}",
                env.loop_info.uw_depth, env.loop_info.sp, env.loop_info.pc, env.loop_info.break_pc
            );
            env.dump_stack();
            println!();
        }
...
mod tests {
    ...
    fn vm_pair() {
        ...
        assert_eq!(gen("$p($p(1,2),$p($p(3,4),3))"), "$p($p(1,2),$p($p(3,4),3))");
    }
    ...
    fn vm_vars() {
        ...
        assert_eq!(gen("!:global x = 10; !y = 11; $:_"), "$&&14.4");
        assert_eq!(
            gen("!:global x = 10; !y = 11; $p(_, $p(_1, $p(_2, y)))"),
            "$p(14.4,$p(10,$p($n,11)))"
        );
    }
    ...
    fn vm_check_bytes_impl() {
        ...
        assert_eq!(
            gen("std:bytes:from_vec ~ std:bytes:to_vec $b\"abcdef\""),
            "$b\"abcdef\"",
            "bytes:from_vec"
        );
        ...
        assert_eq!(
            gen("std:bytes:from_hex ~ std:bytes:to_hex $b\"abc\\xFF\" 6 \":\""),
            "$b\"abc\\xFF\""
        );
        assert_eq!(
            gen("std:bytes:from_hex ~ std:bytes:to_hex $b\"abc\\xFF\" 1 \":\""),
            "$b\"abc\\xFF\""
        );
        assert_eq!(
            gen("std:bytes:from_hex ~ std:bytes:to_hex $b\"\\x00abc\\xFF\" 1 \":\""),
            "$b\"\\0abc\\xFF\""
        );
