    ...
    for (item, _) in fmt.iter() {
        ...
        item.at(0).unwrap().with_s_ref(|syn| {
            match syn {
                ...
                _ => {
                    let arg = arg.at(0).unwrap().with_s_ref(|arg_syn| match arg_syn {
                        "index" => {
                    ...
                    fmts.push(compile_format(arg, &item.at(2).unwrap_or(VVal::None)));
...
pub fn create_formatter_fun(fmt: &VVal) -> Result<VVal, ParseError> {
    let (is_bytes, fmt_str) = if let VVal::Byt(bytes) = fmt {
    ...
    Ok(if is_bytes {
        VValFun::new_fun(
            move |env: &mut Env, _argc: usize| {
                ...
                let mut fs = FormatState { str_data: None, byte_data: Some(out) };
                match (fun)(&mut fs, env.argv_ref()) {
                    Err(e) => Ok(env.new_err(format!("Formatter error: {}", e))),
        ...
        VValFun::new_fun(
            move |env: &mut Env, _argc: usize| {
                let out = String::new();
                let mut fs = FormatState { str_data: Some(out), byte_data: None };
                match (fun)(&mut fs, env.argv_ref()) {
                    Err(e) => Ok(env.new_err(format!("Formatter error: {}", e))),
