...
pub enum ChemFormula {
    Element(u8, u32),
    Group(Vec<ChemFormula>, u32),
    Ion(Vec<ChemFormula>, u32),
}
...
fn try_parse_element(ps: &mut State) -> Option<u8> {
    ...
    let num = match s.at(0) {
        ...
        'B' | 'b' => Some((1, 5)),  // B: Boron
        'C' | 'c' => Some((1, 6)),  // C: Carbon
        'F' | 'f' => Some((1, 9)),  // F: Fluorine
        'H' | 'h' => Some((1, 1)),  // H: Hydrogen
        'I' | 'i' => Some((1, 53)), // I: Iodine
        'K' | 'k' => Some((1, 19)), // K: Potassium
        'N' | 'n' => Some((1, 7)),  // N: Nitrogen
        'O' | 'o' => Some((1, 8)),  // O: Oxygen
        'P' | 'p' => Some((1, 15)), // P: Phosphorus
        'S' | 's' => Some((1, 16)), // S: Sulfur
        'U' | 'u' => Some((1, 92)), // U: Uranium
        'V' | 'v' => Some((1, 23)), // V: Vanadium
        'W' | 'w' => Some((1, 74)), // W: Tungsten
        'Y' | 'y' => Some((1, 39)), // Y: Yttrium
        _ => None,
    };
    ...
    match peek_char {
        '0'..='9' => {
            ...
            if let Ok(cn) = u32::from_str_radix(&num, 10) {
                ...
                Err(ps.err(ParseErrorKind::UnexpectedToken(format!(
...
fn parse_formula(ps: &mut State, end_char: char) -> Result<ChemFormula, ParseError> {
    while ps.peek().unwrap_or(end_char) != end_char {
        match ps.expect_some(ps.peek())? {
            '(' => {
                ps.consume_wsc();
                let expr = parse_formula(ps, ')')?;
                if !ps.consume_if_eq_wsc(')') {
                    return Err(ps.err(ParseErrorKind::ExpectedToken(')', "sub formula end")));
                }
                if let Some(num) = try_parse_number(&mut ps)? {}
                Ok(expr)
            }
            '[' => {
                ps.consume_wsc();
                let expr = parse_formula(ps, ']')?;
                if !ps.consume_if_eq_wsc(']') {
                    return Err(ps.err(ParseErrorKind::ExpectedToken(']', "ion formula end")));
                }
                if let Some(num) = try_parse_number(&mut ps)? {}
                Ok(expr)
            }
            _ => if let Some(elem) = try_parse_element(&mut ps) {},
        }
    }
}
...
pub fn parse_chemical_sum_formula(s: &str) -> Result<VVal, ParseError> {
    ...
    let formula = parse_formula(&mut ps);
