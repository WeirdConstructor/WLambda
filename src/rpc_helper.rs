                ...
                if let VVal::Usr(mut resp) = resp {
                    ...
                    name.with_s_ref(|name| {
                        if let Some(v) = ctx.get_global_var(name) {
                            let arg = if args.is_none() { vec![] } else { args.to_vec() };
                            let ret = match ctx.call(&v, &arg) {
                                Ok(v) => v,
                                Err(e) => {
                                    VVal::err_msg(&format!("Panic in call to '{}': {:?}", name, e))
                                }
                            };
                            resp.send(&ret);
                        } else {
                            resp.send(&VVal::err_msg(&format!("No such global on call: {}", name)));
                        }
                    });
                } else {
                ...
                name.with_s_ref(|name| {
                    if let Some(v) = ctx.get_global_var(name) {
                        ...
                        if ret.is_err() {
                            handle
                                .error_channel
                                .send(&VVal::err_msg(&format!("Error on send: {}", ret.s())));
                        }
                    } else {
                        handle
                            .error_channel
                            .send(&VVal::err_msg(&format!("No such global on send: {}", name)));
                    }
                });
            }
            _ => (),
